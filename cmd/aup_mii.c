...
/* ---------------------------------------------------------------- */
static int do_aup_mii(cmd_tbl_t *cmdtp, int flag, int argc, char * const argv[])
{
	char		op[2];
	unsigned char	addrlo, addrhi, reglo, reghi;
	unsigned char	addr, reg, page;
	...
	if ((flag & CMD_FLAG_REPEAT) == 0) {
		...
		if (argc >= 7)
			mask = simple_strtoul(argv[6], NULL, 16);
	}
    // printf("phy: %u-%u page: %u reg: %u-%u\n", addrlo, addrhi, page, reglo, reghi);
    ...
    int page_def = 0;/* for save original page */
	/*
	...
	if (op[0] == 'i') {
		...
		for (addr = addrlo; addr <= addrhi; addr++) {
            if ( miiphy_read(devname, addr, 22, &page_def) != 0 ) {
                printf("read original page failed\n");
                break;
            }
            if ( miiphy_write(devname, addr, 22, page) != 0 ) {
                printf("setup page %#x failed\n", page);
            }
            udelay(10000);
            ...
            if ( miiphy_write(devname, addr, 22, page_def) != 0 ){
                printf("error in roll back to the orignal page\n");
            }
			if ((addrlo != addrhi) && (reglo != reghi))
				printf("\n");
		}
...
U_BOOT_CMD(
	aup_mii, 7, 1, do_aup_mii,
	"AUPERA MII utility commands",
	"aup_mii device                                   - list available devices\n"
	"aup_mii device <devname>                         - set current device\n"
	"aup_mii info   <addr> <page>                     - display MII PHY info\n"
	"aup_mii read   <addr> <page> <reg>               - read  MII PHY <addr> register <reg>\n"
	"aup_mii write  <addr> <page> <reg> <data>        - write MII PHY <addr> register <reg>\n"
	"aup_mii modify <addr> <page> <reg> <data> <mask> - modify MII PHY <addr> register <reg>\n"
	"                                                   updating bits identified in <mask>\n"
	"aup_mii dump   <addr> <page> <reg>               - pretty-print <addr> <reg> (0-5 only)\n"
